...
def load_citeseer_data(data_path="/home/sinon/dataset/Planetoid"):
    """
    使用 PyTorch Geometric 加载 CiteSeer 数据集。

    参数:
        data_path (str): 数据存储目录，默认为 "/home/sinon/dataset/Planetoid/CiteSeer"
    ...
    data = dataset[0]  # Planetoid 返回一个列表，CiteSeer 只有一个图
    ...
    train_mask = data.train_mask  # 默认 120 个训练节点
    val_mask = data.val_mask  # 默认 500 个验证节点
    test_mask = data.test_mask  # 默认 1000 个测试节点

    print(f"Loaded CiteSeer data: {data.num_nodes} nodes, {data.num_edges} edges, "
